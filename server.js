...
app.post('/api/guardar-borrador', (req, res) => {
  try {
    ...
    let filename;
    if (nombre) {
      const safeName = nombre.replace(/[^a-zA-Z0-9áéíóúñÁÉÍÓÚÑ\s-_]/g, '').substring(0, 50);
      filename = `${safeName}.json`;
    } else {
      const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
      const causante = formData.causante || {};
      const nombreCausante = `${causante.primer_apellido || ''}_${causante.nombres || ''}`.replace(/\s+/g, '_') || 'sin_nombre';
    ...
    if (!fs.existsSync(path.join(__dirname, 'borradores'))) {
      fs.mkdirSync(path.join(__dirname, 'borradores'), { recursive: true });
    }
